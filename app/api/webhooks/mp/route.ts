            ...
            if (externalRef) {
                try {
                    ...
                    if (status === 'approved' && order.items.length > 0) {
                        // Usar la fecha de aprobación del pago de MercadoPago
                            ...
                            if (!existingSale) {
                                await prisma.sale.create({
                                    data: {
                                        ...
                                        date: paymentDate, // ✅ Fecha real del pago
                                        note: `Venta online MP - Orden #${order.id.slice(0, 8)} | MP:${order.id}:${item.id}`,
                                        channel: 'online',
                                    },
                                });
                                console.log(`[MP Webhook] Sale creado para item ${item.nameSnapshot} - Orden ${order.id.slice(0, 8)}`);
                            }
                        }
                    }
                } catch (e) {
